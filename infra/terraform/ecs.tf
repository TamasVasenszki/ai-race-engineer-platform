...
# Execution role: pulls the image, writes logs, reads the DB secret.
...
# Task role: the app itself calls no AWS APIs, so it stays empty (least privilege).
...
resource "aws_security_group" "ecs" {
  name        = "${local.name}-ecs"
  description = "Backend tasks: traffic from the ALB only."
  vpc_id      = module.vpc.vpc_id
...
resource "aws_ecs_service" "backend" {
  ...
  # CD updates the task definition out of band; don't let Terraform revert it.
  lifecycle {
